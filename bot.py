...
def create_request(message: telebot.types.Message) -> None:
    ...
    if (gpt.count_tokens("Ты сценарист придумывающий историю для людей, помогай им, но делай интригу в конце,"
                         "чтобы человек смог её продолжить" + f"Привет сделай историю в жанре {genre},"
                                                              f" в которой персонаж {character}, а сам находится в {
                                                              env} всленной. Сделай интригу чтобы я смог продолжить")
            + 100 > account[2]):
